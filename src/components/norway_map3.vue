<template>
  <div>
    <h3>散点聚集</h3>
    <papa_parse @getGeoData="setScatter"></papa_parse>
    <button @click="drawMap">drawMap</button>
    <div id="tooltip" class="tooltip"></div>
    <div ref="Map" id="main"></div>
  </div>
</template>


<script>
import * as echarts from 'echarts'
// eslint-disable-next-line no-unused-vars
import NorwayJSON from "../assets/json/norway-with-regions_.json"
// eslint-disable-next-line no-unused-vars
import NorwaymapJson1 from '../assets/json/NorwayMap.json'
// eslint-disable-next-line no-unused-vars
import mapJson2 from '../assets/json/kommuner_komprimert.json'
import papa_parse from "@/components/papa_parse";
export default {
  name: "Norway_map3",
  components:{
    papa_parse,
  },
  data(){
    return{
      regions:[
        {
          name:"Troms",
          itemStyle: {
            areaColor: '#374ba4',
            opacity: 1,
          }
        },
        {
          name:'Aust-Agder',
          itemStyle: {
            areaColor: '#fe9b45',
            opacity: 1,
          },
        },
        {
          name: 'Buskerud',
          itemStyle: {
            areaColor: '#ffc556',
            opacity: 1,
          },
        },
      ],

      scatter:[
        {name:'1',value:[11.5358, 58.868,9],rubType:{personal_use: 20, fisheries: 39, industrial: 20, hygiene: 0, others: 12},symbolSize: 10},
        { name: 'Point 1', value: [12.5358, 56.868,9], rubType: { personal_use: 10, fisheries: 5, industrial: 20, hygiene: 8, others: 15 } },
        { name: 'Point 2', value: [10.5358, 59.868,9], rubType: { personal_use: 5, fisheries: 12, industrial: 18, hygiene: 7, others: 10 } },
        // ... 更多散点数据
      ],
      chartID:'bar-chart-' + Math.random().toString(36).substr(2, 9),
      zoomCounter:5,

    }
  },
  mounted() {
    this.darwMap()
  },
  update(){
    this.darwMap()
  },
  methods:{
    darwMap(){
      //初始化一个echarts容器
      const myChart=echarts.init(this.$refs.Map);
      echarts.registerMap('norway',NorwaymapJson1)
      // eslint-disable-next-line no-unused-vars
      const randomColor=this.getRandomColor();
      // eslint-disable-next-line no-unused-vars
      var colorPalette = echarts.color.palette;
      var geo_labelFontSize=10;
      var distanceThreshold=1.5
      let hasExecutedLowerZoomOption = false; // 用于跟踪是否已经执行过低缩放级别的 setOption
      let hasExecutedHigherZoomOption = false; // 用于跟踪是否已经执行过高缩放级别的 setOption
      // eslint-disable-next-line no-unused-vars
      myChart.on('georoam',  (params)=> {
        //如果是拖拽事件则退出
        if (params.dy || params.dx) return;
        //获取最新的option配制
        let nowOption=myChart.getOption()
        let zoomLevel = nowOption.geo[0].zoom;
        //获取当前的缩放比例
        console.log('zoomLevel',zoomLevel)
        if (zoomLevel < 8 && !hasExecutedLowerZoomOption) {
          distanceThreshold=this.updataDistanceThreshold(zoomLevel,1.5)
          //实现垃圾marker聚类
          var clusteredScatterData= this.scatterAggregation(myChart,distanceThreshold)
          nowOption.series[0].data=clusteredScatterData
          //清空
          myChart.clear();
          // 在这里执行低缩放级别的 setOption 操作
          myChart.setOption(nowOption); // 替换为你的低缩放级别的新 option
          hasExecutedLowerZoomOption = true; // 标记已执行
          hasExecutedHigherZoomOption=false;
        }

        // 高缩放级别的条件（大于 zoomlevel=45）
        if (zoomLevel > 35 && !hasExecutedHigherZoomOption) {
          distanceThreshold=this.updataDistanceThreshold(zoomLevel,1.5)
          //实现垃圾marker聚类
         // clusteredScatterData= this.scatterAggregation(myChart,distanceThreshold)
          nowOption.series[0].data=this.scatter
          myChart.clear()
          // 在这里执行高缩放级别的 setOption 操作
          myChart.setOption(nowOption); // 替换为你的高缩放级别的新 option
          hasExecutedHigherZoomOption = true; // 标记已执行
          hasExecutedLowerZoomOption=false;
        }
      });
      window.addEventListener('resize', function () {
        myChart.resize()
      })

      var option={
        geo:{
          type: 'map',
          map:'norway',//用上述注册地图
          roam: true,//是否允许缩放
          zoom:10,//初始化大小
          scaleLimit:{
            min:1,
            max:500,
          },
          label:{
            show:true,
            fontSize:geo_labelFontSize
          },
          //设置中心点
          center:[6.595407938897836,
            67.06621414564643],
          //省份地图添加背景
          regions: this.regions,
          itemStyle: {
            areaColor: 'white',
            //areaColor:{},
            color: 'red',
            borderColor: '#232652',
            borderWidth: 2,
          },
          //高亮状态
          emphasis: {
            itemStyle: {
              areaColor: '#1af',
              color: '#fff',
            },
          },
        },
        tooltip:{
          trigger:"item",
          formatter(params){
            if(params.componentType==='series'&& params.seriesType==='scatter'){
              console.log('params:',params.data.value)
              const data = params.data; // 获取选中的散点数据
              const rubType = data.rubType; // 获取 rubType 对象
              const rubTypeKeys = ['personal_use', 'fisheries', 'industrial', 'hygiene', 'others'];
              const barData = rubTypeKeys.map(key => rubType[key]);


              //简单样式输出
              var result='';
              for (var i=0;i<5;i++){
                result+=rubTypeKeys[i]+': '+barData[i]+'<br />'
              }
              return result


            }
            else return ""
          }
        },
        //配置属性
        series:[{
          type:'scatter',
          //type: 'effectScatter',
          coordinateSystem: 'geo',
          data: this.scatter,
          showEffectOn: 'render',
          rippleEffect: {
            //涟漪特效相关配置
            brushType: 'stroke', //波纹的绘制方式，可选 'stroke' 和 'fill'
          },
          hoverAnimation: true, //是否开启鼠标 hover 的提示动画效果
          label: {
            //图形上的文本标签，可用于说明图形的一些数据信息，比如值，名称等，
            formatter: '{b}',
            position: 'right',
            show: true,
            textStyle: {
              // 设置标签的字体样式
              color: 'black', // 设置标签字体颜色（淡色）
              fontSize: 10, // 设置字体大小
              fontWeight: 'normal', // 设置字体粗细 bold,normal
            },
            color:'black'
          },
          itemStyle: {
            //图形样式，normal 是图形在默认状态下的样式；emphasis 是图形在高亮状态下的样式，比如在鼠标悬浮或者图例联动高亮时
            //color:'white', //散点的颜色
            show: true,
            // shadowBlur: 0,
            // shadowColor:'transparent',
          },

          symbolSize: 10, // 设置散点的大小
          zlevel: 1,
          symbol: "image://data:image/png;base64,"
        }],

      }
      myChart.setOption(option)


    },
    setScatter(data){
      console.log("map_object:    ",data)
      console.log(data.length)
      for(var i=0;i<data.length;i++){
        var tem={
          name:data[i].name,
          value:data[i].value,
          rubType:data[i].type,
          kg_of_collected_waste:data[i].kg_of_collected_waste,
          county_name:data[i].county_name
        }
        this.scatter.push(tem)
      }
      console.log("scatter:   ",this.scatter)
    },
    getRandomColor(){
      // 生成随机的 R、G 和 B 分量
      const r = Math.floor(Math.random() * 256); // 0 到 255
      const g = Math.floor(Math.random() * 256);
      const b = Math.floor(Math.random() * 256);

      // 将分量转换为十六进制，并保证字符串长度为 2
      const rHex = r.toString(16).padStart(2, '0');
      const gHex = g.toString(16).padStart(2, '0');
      const bHex = b.toString(16).padStart(2, '0');

      // 返回随机颜色的字符串表示，格式为 '#RRGGBB'
      return `#${rHex}${gHex}${bHex}`;
    },
    // 计算标签字体大小的函数，根据需要自行定义
    calculateLabelFontSize(zoomLevel) {
      const scale = (zoomLevel - 1) / (80 - 1);

      // 根据缩放比例调整字体大小
      const minFontSize = 5; // 最小字体大小
      const maxFontSize = 45; // 最大字体大小
      const fontSizeRange = maxFontSize - minFontSize; // 字体大小范围

      // 计算当前缩放级别对应的字体大小
      var currentFontSize = minFontSize + scale * fontSizeRange;
      if(zoomLevel<2){
        currentFontSize=1
      }
      return currentFontSize;
    },
    autoTextfontsize(zoomLevel){
      // 获取当前地图缩放级别
      //let zoomLevel = myChart.getOption().geo[0].zoom;
      // 根据缩放级别调整标签的字体大小
      var geo_labelFontSize=10
      geo_labelFontSize = this.calculateLabelFontSize(zoomLevel);
      console.log("geo_labelFontSize",geo_labelFontSize)
      return geo_labelFontSize
    },
    //数据更新后一直保留了，要改！！
    scatterAggregation(myChart,distanceThreshold) {
      //开始
      console.log('开始')
      var scatterData = this.scatter
      console.log("scatterData",scatterData)
      const clusteredScatterData = [];
      console.log('clusteredScatterData:',clusteredScatterData)
      scatterData.forEach(point => {
        console.log("point: ",point.rubType)
        let isClustered = false;

        for (const cluster of clusteredScatterData) {
          console.log('cluster1:',cluster.rubType)
          console.log('distance: ',this.calculateDistance(point, cluster))
          if (this.calculateDistance(point, cluster) < distanceThreshold) {
            // 如果距离小于阈值，将点合并到聚集中
            for (const rubType in point.rubType) {
              console.log('rubType',rubType)
              if (cluster.rubType[rubType] && point.rubType[rubType]) {
                cluster.rubType[rubType] += point.rubType[rubType];
              } else if (point.rubType[rubType]) {
                cluster.rubType[rubType] = point.rubType[rubType];
              }
              console.log('cluster2:',cluster.rubType)
            }
            isClustered = true;
            break;
          }
        }

        if (!isClustered) {
          // 如果无法与任何现有聚集合并，添加为新聚集
          clusteredScatterData.push(point);

        }
      });
      return clusteredScatterData
    },
    calculateDistance(point1, point2) {
      // 使用经纬度坐标计算两点之间的距离
      const [x1, y1] = point1.value;
      const [x2, y2] = point2.value;
      const deltaX = x2 - x1;
      const deltaY = y2 - y1;
      return Math.sqrt(deltaX * deltaX + deltaY * deltaY);
    },
    updataDistanceThreshold(zoomLevel,oldDistanceThreshold){
      var distanceThreshold=oldDistanceThreshold
      if(zoomLevel<8)
      {
        distanceThreshold=2;
      }
      else if(zoomLevel>25)
      {
        distanceThreshold=0.5
      }
      else distanceThreshold=oldDistanceThreshold;
      return distanceThreshold;
    }
  }
}
</script>

<style scoped>
#main{
  width: 1000px;
  height: 800px;
  border: 1px solid red;
}
</style>